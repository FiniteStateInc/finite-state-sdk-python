"""
get_fda_components_metadata.py
...
def get_components_for_artifact_version(token, organization_context, artifact_id, version_id, component_type=None, min_risk_score=None, verbose=False, max_retries=3):
    ...
    # Allowed component types for FDA report
    ALLOWED_TYPES = {"operating_system", "package", "library"}

    filtered = []
    for comp in components:
        status = (comp.get('currentStatus') or {}).get('status', None)
        comp_type_normalized = (comp.get('type') or '').lower()
        risk_score = comp.get('absoluteRiskScore', None)

        # Status/type exclusions
        if status in ('REVIEWED_FALSE_POSITIVE', 'EXCLUDED'):
            continue

        # Keep only allowed types
        if comp_type_normalized not in ALLOWED_TYPES:
            continue

        if component_type and comp_type_normalized != component_type.lower():
            continue
...
def export_to_csv(components, output_file, verbose=False):
    ...
    # Only include columns explicitly listed in preferred_order
    columns = preferred_order

    # Custom properties no longer exported since we restrict to preferred columns.
